...
async function main() {
  ...
  try {
    ...
    const sdkPaths = [path.join(__dirname, '../utils/researchClient.js'), path.join(__dirname, 'utils/researchClient.js')]
    const sdkPath = sdkPaths.find(filename => fs.existsSync(filename))
    assert.ok(sdkPath)
    const { createResearchClient } = require(sdkPath)
    ...
    let firstAck = null
    let replayAck = null
    const makeClient = transport => createResearchClient({ storage, transport, now: () => clock, random: () => 0,
