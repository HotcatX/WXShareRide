...
- Joined/left/cancelled/removed notifications, ratings and matched-group rating invitations are atomic with the ride write. Completion accounting has an internal transactional job; public baseline import, display projections and production scheduling remain incomplete. Legacy templates, blocks, notifications, ratings and personal completion receipts have pure converters, but no production import has occurred. Authenticated participant and personal-list projections are separate modules; public responses must never substitute for those authorization rules. Weekly templates reuse `placeSchema`, `offerFieldsSchema` and `orderedStopsSchema`, with their own relative local-time representation.
