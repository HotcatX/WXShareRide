...
- The configured `placeBusinessFiveMinutes` Timer runs every five minutes, reads pending business outbox events, signs and sends them to the collector's `/internal/v1/places/business-events`, then acknowledges accepted events. It accepts the genuine platform trigger with optional `action:'placeBusinessTimer'`, source `wx_trigger`, and no caller/from-user identity.
- Direct handling of a genuine platform Timer with `Type:'Timer'` (or `timer`) and `TriggerName:'publicStatsHourly'`, optionally carrying `action:'publicStatsHourlyTimer'`, remains supported for a later coordinated transfer of the hourly scheduler. It is not the trigger currently configured on this function. The current parsed source must equal `wx_trigger` and must have no caller/from-user identity. A mini-program fabricating Timer fields cannot authorize this path.
...
`statistics/config.json` configures **`placeBusinessFiveMinutes` every five minutes** for business-event synchronization. This trigger does not refresh the public-statistics snapshot.

`syncPublicStatsReplica/config.json` retains the separate **`publicStatsHourly` timer at minute 25**. That function verifies the original trigger, signs the relay and delegates public snapshot refresh to `statistics`; it contains no active database read/publish implementation. The compatibility path adds one invocation per hourly run. Do not remove the old hourly function/trigger while it is the active scheduler, or enable another hourly trigger on `statistics` before a coordinated transfer. The five-minute business trigger remains independent of that transfer.
