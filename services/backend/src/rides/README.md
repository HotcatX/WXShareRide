...
- Joined/left/cancelled/removed notifications are atomic with the ride write. Ratings and rating invitations, completion accounting and full old field mapping remain incomplete. Legacy templates, blocks and notifications now have pure converters, but no production import has occurred. Authenticated participant and personal-list projections are separate modules; public responses must never substitute for those authorization rules. Weekly templates reuse `placeSchema`, `offerFieldsSchema` and `orderedStopsSchema`, with their own relative local-time representation.
