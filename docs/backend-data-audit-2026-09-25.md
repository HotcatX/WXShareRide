...
**这是在线数据库的只读审计提取，不是原子快照，也不是全 48 集合的完整备份。** 本文记录提取时的审计结果；当时 normalizer 为 `ready=false`。后续8个核心集合已通过转换和本地隔离数据库导入演练，完整32集合仍未全部完成迁移，不能据本批提取直接切主。当前契约见 [SCHEMA](../services/backend/SCHEMA.md)，线上权威库仍为 CloudBase。
...
用户、行程、成员、模板、通知、拉黑、评分/完成记录和公共基线的映射及核心导入演练已完成，旧元数据校验后留来源归档，不再生成重复业务字段。后续从以下边界继续：

1. 明确市场、文件、配置、行为及仅盘点集合的迁移、归档或重建策略；核心集合通过不代表整个产品通过。
2. 完成剩余功能、原附件内容和权限、旧客户端兼容及恢复验证；原始库保持权威。
3. 正式切换前生成新的完整导出并追平持续写入，验证幂等与回滚；本批数据只作格式审计及迁移开发依据。
