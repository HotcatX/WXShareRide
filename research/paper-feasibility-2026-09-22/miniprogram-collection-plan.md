...
> 档案说明（2026-09-26）：本页保留研究测量设计，不是当前页面、接口或部署任务。旧实施代理提示词的独有结果语义已合并到本页；当前代码和协议使用[采集服务](../../services/analytics-collector/README.md)及[后端模型](../../services/backend/SCHEMA.md)。旧建议中的新按钮、参与管理入口、字段名和上传频率不能覆盖后续产品决定；现有产品要求不新增隐私引导或采集开关，回访保持是／否且不追问金额。未实施的实验测量仍需单独确定，不能通过重新照抄本页宣称已采齐。
...
| 旧原因页面 | [tripActionReason.js:204](https://github.com/HotcatX/WXShareRide/blob/88a9221/pages/profile/tripActionReason/tripActionReason.js#L204) 先客户端写 `TripActions`，235行才调用云函数，云函数又可能写日志 | 某些事件可能表示尝试而非成功，存在重复且时间字段有 `createTime/createdAt` 差异；旧日志不能不加清洗直接算取消率 |
...
研究首问为“这次预约，您实际坐上这趟车了吗？”的**是／否双按钮，另有独立关闭入口**；绑定明确预约对象，不从最后浏览路线关联。只有明确回答并得到服务端确认才算已保存；关闭、未回复、失败和延期均为未知。“是”仅表示自报乘上该车，不证明到达、准点或付款；“否”不等于没有通过其他渠道出行。司机按具体预约乘客回答，不能用整趟车出发确认所有人。

以下保留旧实施提示词中的研究区分，不要求新增界面或沿用旧接口名：

- 回答对象是预约账号本人（`subjectScope=bookingAccountPerson`）；一个账号代订多人时，不把yes乘以预约人数。司机无法确认本人或代订不明时保留未知；未来全组人数需独立问题，不能改变旧答案含义。
- 双边答案只在同一乘客、司机/车辆和服务时次内合并，保留双方来源、修订历史和冲突；不要向对方显示答案或与评分奖惩挂钩。合并层级可区分双方肯定/双方否定/单方报告/冲突/未知。
- 回访资格、领取、已展示、关闭、回答及自动邀请过期分别记录；领取不等于展示。多端需协调账号当日额度与任务租约，未收到shown回包不能断言没显示，记展示不确定并保守限制重复邀请。
- 历史草案采用纽约服务日期次日09:00、每次前台及每日最多一项、单对象最多两次且间隔24小时、7天后不自动邀请。这些只是未验证的产品参数，不是旅行已结束或统计充分的证据；自动邀请截止也不等于授权内手动更正截止。
- 本人×行程×对方×服务时次去重，并关联重加等参与段；改期、换司机需明确归属。延期、更正和撤销应保留版本，延期不改业务发车安排；切号、隐藏和旧异步响应不能错误弹出。
...
调用与容量的历史论证统一见[调用模型](independent-collector-and-call-model.md)。研究上需要有界本地队列、稳定事件/批次标识、确认后出队和可观测丢失；不能依赖后台或最后一次退出必达。原先“随已有请求附带、每会话最多一次独立补传、间隔5分钟”是CloudBase阶段的预算方案，并非当前独立接收器配置；现行批次、频率和重试以采集服务README及代码为准，不在本文再维护第二套参数。
