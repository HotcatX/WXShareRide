...
export async function getRideParticipants(pool: Pool, userId: string, id: unknown): Promise<Participants> {
  ...
  const result = await pool.query<Omit<Participants, 'participants' | 'largeLuggageCount'> & {
    participants: ParticipantRow[]; largeLuggageCount: unknown;
  }>(`
  ...
  const { largeLuggageCount, participants, ...ride } = row;
  const data = { ...ride, participants: participants.map(member => ({ ...member, statistics: formatStatistics(member.statistics) })) };
