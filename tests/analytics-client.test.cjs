...
const { createAnalyticsClient, createWxTransport, validEndpoint, STORAGE_KEY } = require('../utils/analyticsClient')
const { sha256, utf8ByteLength } = require('../utils/hash')
...
test('renamed client resumes a pre-upgrade queue without changing its persisted wire body', async () => {
  // Literal deployed values deliberately differ from importing today's constants:
  // changing a constant must not make this migration regression silently pass.
  const event = { eventId: 'legacy_event_0000001', eventName: 'page_view', schemaVersion: 1,
    occurredAt: T, sessionId: 'legacy_session_00001', data: { page: 'home' } }
  const body = JSON.stringify({ schemaVersion: 1, batchId: 'legacy_batch_000001', events: [event] })
  const map = new Map([['ride_research_queue_v1', {
    schemaVersion: 1, participantKey: 'participant_00000001', grantId: 'consent_grant_000001',
    statusVersion: 1, purposeVersion: 'ride-research-v1', events: [event],
    pending: { batchId: 'legacy_batch_000001', body, payloadHash: sha256(body), eventIds: [event.eventId] },
    failureCount: 1, nextAttemptAt: T - 1, lastAttemptAt: T - 300000, halted: '',
    dropped: { expired: 0, queueLimit: 0, invalid: 0 }
  }]])
  const h = harness({ map })
  h.start(grant({ purposeVersion: 'ride-research-v1', acceptedPurposeVersion: 'ride-research-v1' }))
  assert.equal(h.client.getStatus().pendingCount, 1)
  assert.equal((await h.client.flush()).eventCount, 1)
  assert.equal(h.calls[0].body, body)
  assert.deepEqual([...map.keys()], ['ride_research_queue_v1'])
  assert.equal(map.get('ride_research_queue_v1').events.length, 0)
})
