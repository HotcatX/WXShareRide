...
只挂 socket目录、admin.token、两个脚本及兼容常量模块，不挂数据库、subject密钥、Docker socket或网络。`DAC_OVERRIDE` 仅用于连接现有 UID1000/0600 socket并读取同样受保护的 admin.token，工具仍强制验证文件所有权、权限和类型；不放宽服务现有权限。脚本复用 stop 工具的输入校验函数，导入不会执行停止操作。
