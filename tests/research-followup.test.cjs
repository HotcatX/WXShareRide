...
const { createFollowupController, eligibleTrip, referencePrice, STORAGE_PREFIX } = require('../utils/researchFollowup')
...
function harness(options = {}) {
  ...
  const controller = createFollowupController({ wx, research, now: () => state.now })
  const page = { data: {}, setData(patch) { Object.assign(this.data, copy(patch)) } }
  return { controller, page, store, state, wx, research }
}
