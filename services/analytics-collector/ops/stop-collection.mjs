...
export const ENDPOINT = `http://127.0.0.1:3000${BRIDGE_ROUTE}`;
...
export function deriveSubject(openid, subjectKey) {
  ...
  return createHmac('sha256', subjectKey).update(`${SUBJECT_SCOPE}\n${APPID}\n${openid}`).digest('hex');
}
...
function readStatus(reply) {
  if (!object(reply) || reply.ok !== true || !['none', 'active', 'revoked'].includes(reply.status) ||
    !version(reply.statusVersion) || reply.purposeVersion !== DEFAULT_PURPOSE_VERSION || reply.noticeVersion !== DEFAULT_NOTICE_VERSION) fail('INVALID_RESPONSE');
...
export async function stopCollection({ openid, subjectKey, bridgeKey, transport = sendLocal,
  ...
  try {
    ...
    const payload = (action, expectedStatusVersion) => {
      ...
      return { accountSubject, action, requestId, expectedStatusVersion, purposeVersion: DEFAULT_PURPOSE_VERSION, noticeVersion: DEFAULT_NOTICE_VERSION };
    };
