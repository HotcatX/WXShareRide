  ...
  for (const percent of [0, 5, 100]) {
    for (const bucket of [0, 499, 500, 9999]) {
      ...
      for (let reopen = 0; reopen < 2; reopen++) {
        ...
        assert.equal(context.serverEnabled, expected, `${percent}% / ${bucket}`)
...
test('a new anonymous bucket is persisted once, survives reopening, and never appears in HTTP or diagnostics', async () => {
  ...
  const context = h.api.getPublicStatsReadContext()
  assert.equal(context.serverEnabled, true)
  ...
  assert.equal(reopened.api.getPublicStatsReadContext().serverEnabled, true)
  ...
  assert.equal(JSON.stringify(h.api.getPublicStatsDiagnostics()).includes('bucket'), false)
})
...
test('valid snapshot merges concurrent reads, uses fixed HTTPS without credentials, and reports safe counters', async () => {
  ...
  assert.deepEqual(plain(h.api.getPublicStatsDiagnostics()), { httpRequests: 1, cloudRequests: 0, fallbacks: 0, discarded: 0, circuitSkips: 0 })
  const counts = h.api.getPublicStatsDiagnostics(); counts.httpRequests = 999
  assert.equal(h.api.getPublicStatsDiagnostics().httpRequests, 1)
})
...
test('full schema, dates, six-hour lifetime and exact data hash are validated before accepting a snapshot', async () => {
  ...
  for (const change of invalid) {
    ...
    assert.equal(h.state.cloud.length, 1)
    assert.equal(h.api.getPublicStatsDiagnostics().fallbacks, 1)
  }
})
...
test('disabling the configuration during HTTP discards and aborts a late server result; a new read uses cloud', async () => {
  ...
  assert.equal(h.state.aborts, 1)
  assert.equal(h.state.cloud.length, 1)
  assert.equal(h.api.getPublicStatsDiagnostics().fallbacks, 0)
})
...
test('two consecutive HTTP failures cool down for five minutes, then one shared probe can recover', async () => {
  ...
  assert.equal(h.api.getPublicStatsReadContext().serverEnabled, true, 'circuit does not change rollout membership')
  ...
  assert.equal(h.api.getPublicStatsDiagnostics().fallbacks, 2)
  assert.equal(h.api.getPublicStatsDiagnostics().circuitSkips, 2)
  h.state.now++
...
test('concurrent callers count one failed HTTP attempt, while a success breaks the failure streak', async () => {
  ...
  assert.equal(h.api.getPublicStatsDiagnostics().circuitSkips, 0)
})
