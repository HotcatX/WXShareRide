...
当前部署镜像为 `linkx-research-collector:20260923-expanded`。客户端三个环境均配置100%，develop/trial使用独立test命名空间，release使用真实命名空间。本工具固定派生 `linkx-research-account-v1` **真实命名空间**，不支持关闭或清理test授权；不要为清理测试而对同账号运行此工具，否则会影响其真实采集状态。版本 `2026.09.23.2` 已上传但未正式发布；本轮验收来自开发者工具，未宣称手机验证。
...
sudo docker run --rm -i --network host --read-only --cap-drop ALL \
  ...
  -v /opt/linkx-collector/ops/stop-collection.mjs:/stop-collection.mjs:ro \
  --entrypoint node linkx-research-collector:20260923-expanded /stop-collection.mjs
```

只挂载这两个只读路径，不挂载数据库、业务文件或Docker socket。host网络让工具访问宿主loopback的固定服务地址。不要为了管道使用包含原始账号值的 `echo`/shell 命令。stdin 最长等5秒，每次本机请求最多3秒，最多两次请求，不跟随重定向，不自动重试。
...
处理完成后按运维规则清理含原始账号的临时输入文件；普通文件删除不等于所有介质上的安全擦除。不将该文件复制到备份或研究导出。维护用途/说明版本时同步脚本常量；版本不匹配时工具拒绝操作，不尝试绕过校验。
...
本轮开发者工具测试后的清理由部署负责人另行执行：只删除synthetic=1、purpose=ride-research-v1、截至1790190142676的11批/28事件载荷；测试和真实eligible事件均0，保留最小去重收据及1个active测试授权供后续测试。该清理没有使用本停止工具，没有撤销真实或测试身份，也不表示所有控制元数据已删除。
