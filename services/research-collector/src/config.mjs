...
import { loadBridgeKey, DEFAULT_NOTICE_VERSION, isNoticeVersion } from './bridge.mjs';
...
export function readConfig(env = process.env) {
  ...
  const noticeVersion = env.RESEARCH_NOTICE_VERSION || DEFAULT_NOTICE_VERSION;
  if (!isNoticeVersion(noticeVersion)) throw new Error('Invalid research notice version');
  return {
    ...
    bridgeKey: loadBridgeKey(env.RESEARCH_BRIDGE_KEY_FILE), noticeVersion,
    purposeVersion: env.PURPOSE_VERSION || 'ride-research-v1',
