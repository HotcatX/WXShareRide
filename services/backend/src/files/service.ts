...
type FileRow = {
  ...
  owner_user_id: string | null; admin_owner_key: string | null; legacy_readonly: boolean;
...
export type FileRecord = {
  ...
  owner: FileOwner | null; legacyReadonly: boolean; status: FileRow['status'];
...
async function assertOwner(client: PoolClient, appId: string, owner: FileOwner) {
  const result = 'userId' in owner
    ...
    : await client.query('SELECT id FROM admin_accounts WHERE app_id=$1 AND owner_key=$2 AND enabled FOR KEY SHARE', [appId, owner.adminOwnerKey]);
...
export async function reserveFile(client: PoolClient, body: unknown): Promise<FileRecord> {
  ...
  const row = (await client.query<FileRow>(`INSERT INTO files(app_id,provider,locator,owner_user_id,admin_owner_key)
    VALUES($1,$2,$3,$4,$5) ON CONFLICT(provider,locator) DO NOTHING RETURNING *`,
  [input.appId, input.provider, input.locator, 'userId' in input.owner ? input.owner.userId : null,
    'adminOwnerKey' in input.owner ? input.owner.adminOwnerKey : null])).rows[0];
