...
// This pure hash helper does not load or enable the research collection client.
...
function getPublicStatsReadContext() {
  ...
  return { pilotEnabled: selected.enabled, key: `public-stats:${selected.enabled ? 'server' : 'cloud'}:${generation}`, reason: selected.reason }
}

function isPublicStatsReadCurrent(context) {
  const current = getPublicStatsReadContext()
  return !!context && context.key === current.key && context.pilotEnabled === current.pilotEnabled
}
...
function readCloud() {
  ...
  const request = new Promise((resolve, reject) => {
    try {
      ...
      resolve(wx.cloud.callFunction({ name: 'statistics', data: { action: 'publicStats' } }))
...
function loadPublicStats(context = getPublicStatsReadContext()) {
  ...
  entry.promise = (async () => {
    ...
    const coolingDown = context.pilotEnabled && circuitIsOpen()
    if (coolingDown) { reason = 'circuit_open'; counts.circuitSkips += 1 }
    if (context.pilotEnabled && !coolingDown) {
      try {
...
function getPublicStatsPilotDiagnostics() { return Object.assign({}, counts) }

module.exports = {
  getPublicStatsReadContext, isPublicStatsReadCurrent, loadPublicStats, getPublicStatsPilotDiagnostics
}
