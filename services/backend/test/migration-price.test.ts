...
import { parseLegacyListedPrice } from '../src/migration/legacy-price.ts';
...
test('converts established USD listed-price forms without inventing a unit', () => {
  for (const label of ['15.20', '15.20$/人', '$15.20', '15.20$', '15.20 USD', '15.20usd', '15.20美元', '15.20美金', '15.20刀', '$ 15.20 /人', '15.20 USD / 人']) {
    assert.deepEqual(parseLegacyListedPrice(label), { classification: 'priced', cents: 1520, label });
  }
  assert.deepEqual(parseLegacyListedPrice(15.2), { classification: 'priced', cents: 1520, label: '15.2' });
  assert.deepEqual(parseLegacyListedPrice('00015.2'), { classification: 'priced', cents: 1520, label: '00015.2' });
});

test('keeps the exact original string label, while only the recognition copy is trimmed', () => {
  const label = '  $ 12.30  ';
  assert.deepEqual(parseLegacyListedPrice(label), { classification: 'priced', cents: 1230, label });
  const unresolved = '  12-20 USD  ';
  assert.deepEqual(parseLegacyListedPrice(unresolved), { classification: 'unresolved', cents: null, label: unresolved });
});
...
test('uses exact integer cents and enforces the database integer boundary', () => {
  for (const [label, cents] of [['0.29', 29], ['1.01', 101], ['21474836.47$', 2_147_483_647]] as const) {
    assert.deepEqual(parseLegacyListedPrice(label), { classification: 'priced', cents, label });
  }
  for (const label of ['21474836.48$', '99999999.99', '100000000', '0.001', '15.200', '1e2', '1,000', '-1', '+1', '.5', '15.']) {
    assert.deepEqual(parseLegacyListedPrice(label), { classification: 'unresolved', cents: null, label });
  }
});

test('only explicit free labels or zero amounts become zero', () => {
  for (const label of ['免费', 'free', 'FREE', ' 免费 ', '0', '$0.00']) {
    assert.deepEqual(parseLegacyListedPrice(label), { classification: 'priced', cents: 0, label });
  }
  assert.deepEqual(parseLegacyListedPrice(0), { classification: 'priced', cents: 0, label: '0' });
  for (const label of ['不免费', 'free if available', '价格待定', '协商', '司机确认']) {
    assert.deepEqual(parseLegacyListedPrice(label), { classification: 'unpriced', cents: null, label });
  }
});
...
test('missing values and ordinary nonnumeric text do not invent an amount', () => {
  for (const value of [null, undefined]) assert.deepEqual(parseLegacyListedPrice(value), { classification: 'unpriced', cents: null, label: null });
...
test('malformed types cannot stringify themselves into accepted amounts', () => {
  for (const value of [true, false, [], [15], {}, { toString: () => '15' }, new Number(15)]) {
    assert.deepEqual(parseLegacyListedPrice(value), { classification: 'unresolved', cents: null, label: null });
  }
  for (const value of [NaN, Infinity, -Infinity]) {
    assert.deepEqual(parseLegacyListedPrice(value), { classification: 'unresolved', cents: null, label: String(value) });
  }
});
