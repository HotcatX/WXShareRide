...
export function readSafeMetrics(db) {
  ...
  return {
    ...
    realEligibleEvents: count(`SELECT COUNT(*) AS n FROM eligible_events e JOIN ${TABLES.participants} p ON p.participant_key=e.participant_key WHERE p.synthetic=0`),
    ...
    realEventsByName: db.prepare(`SELECT json_extract(e.event_json,'$.eventName') AS eventName, COUNT(*) AS count
      FROM eligible_events e JOIN ${TABLES.participants} p ON p.participant_key=e.participant_key WHERE p.synthetic=0
      GROUP BY eventName ORDER BY eventName`).all(),
    syntheticEligibleEvents: count(`SELECT COUNT(*) AS n FROM eligible_events e JOIN ${TABLES.participants} p ON p.participant_key=e.participant_key WHERE p.synthetic=1`),
    ...
    syntheticEventsByName: db.prepare(`SELECT json_extract(e.event_json,'$.eventName') AS eventName, COUNT(*) AS count
      FROM eligible_events e JOIN ${TABLES.participants} p ON p.participant_key=e.participant_key WHERE p.synthetic=1
      GROUP BY eventName ORDER BY eventName`).all(),
  };
}
