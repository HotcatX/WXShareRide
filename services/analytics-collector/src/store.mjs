...
export function openStore(path, { realEnabled = false, purposeVersion = DEFAULT_PURPOSE_VERSION, maxDatabaseMB = 1024, noticeVersion = DEFAULT_NOTICE_VERSION } = {}) {
  ...
  db.transaction(() => { db.exec(`
    ...
    CREATE VIEW eligible_real_events AS
      SELECT e.* FROM eligible_events e JOIN ${TABLES.participants} p ON p.participant_key=e.participant_key WHERE p.synthetic=0;
  `);
    ...
    if (!db.prepare(`PRAGMA table_info(${TABLES.accounts})`).all().some(column => column.name === 'openid')) {
      db.exec(`ALTER TABLE ${TABLES.accounts} ADD COLUMN openid TEXT`);
    }
    db.exec(`CREATE INDEX IF NOT EXISTS ${TABLES.accountOpenidIndex} ON ${TABLES.accounts}(openid);
  ...
  const stateTransaction = db.transaction((state, now) => {
    ...
    if (state.status === 'active') {
      ...
      const account = db.prepare(`SELECT openid FROM ${TABLES.accounts} WHERE participant_key=?`).get(state.participantKey)?.openid;
      if (account) {
    ...
    db.prepare(`INSERT INTO ${TABLES.participants} VALUES (?,?,?,?,?,?,?) ON CONFLICT(participant_key) DO UPDATE SET
  ...
  const participationTransaction = db.transaction((request, now) => {
    ...
    if (request.openid !== undefined) {
      ...
      requireThat(!db.prepare(`SELECT 1 FROM ${TABLES.accounts} a JOIN ${TABLES.participants} p ON p.participant_key=a.participant_key
    ...
    db.prepare(`INSERT INTO ${TABLES.accounts} (account_subject,participant_key,notice_version,created_at,openid) VALUES (?,?,?,?,?)
  ...
  return {
    ...
    prune(now = Date.now()) {
      return db.transaction(() => {
        const payloads = db.prepare(`DELETE FROM ingest_batches WHERE
          (participant_key IN (SELECT participant_key FROM ${TABLES.participants} WHERE synthetic=1) AND received_at <= ?)
    ...
    recoveryComplete() {
      return db.transaction(() => {
        ...
        const now = Date.now();
        const real = db.prepare(`SELECT * FROM ${TABLES.participants} WHERE synthetic=0`).all();
        db.prepare(`DELETE FROM ingest_batches WHERE participant_key IN (SELECT participant_key FROM ${TABLES.participants} WHERE synthetic=0)`).run();
        ...
        for (const p of real) {
          ...
          db.prepare(`UPDATE ${TABLES.participants} SET status='revoked',status_version=?,updated_at=? WHERE participant_key=?`)
