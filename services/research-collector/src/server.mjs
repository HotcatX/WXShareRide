...
import { BRIDGE_ROUTE, DEFAULT_NOTICE_VERSION, isNoticeVersion, verifyBridgeRequest, validateParticipationRequest } from './bridge.mjs';
...
export function createCollector(config) {
  ...
  const batchHandler = wrap(async (req, res) => {
    if (req.method === 'GET' && req.url === '/healthz') {
      store.db.prepare('SELECT 1').get(); return reply(res, 200, { ok: true, service: 'research-collector' });
    }
  ...
  const adminServer = http.createServer({ maxHeaderSize: 8192 }, wrap(async (req, res) => {
    ...
    if (req.method === 'GET' && req.url === '/v1/status') {
      return reply(res, 200, { ok: true, realCollectionEnabled: config.realEnabled,
        ...
        participants: store.db.prepare('SELECT COUNT(*) AS n FROM research_participants').get().n,
