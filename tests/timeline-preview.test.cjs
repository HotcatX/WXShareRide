...
test('unavailable detail has no retry loop and can return to public listings', async () => {
  ...
  assert.equal(f.instance.data.error.title, '这条信息暂不可查看')
  assert.equal(f.instance.data.error.expiredRoute, undefined)
  f.instance.onFindAvailableTrips()
  assert.equal(f.requests.length, 1)
  f.instance.onRetry()
  assert.equal(f.requests.length, 1)
  f.instance.onBackToList()
...
test('expired shared offers and requests show only the expiry state and open fresh available trip listings', async () => {
  for (const type of ['carpool', 'request']) {
    const f = fixture({ kind: 'trip', type, id: 'ended' })
    f.attach()
    f.requests[0].reject(Object.assign(new Error('not_found'), { code: 'UNAVAILABLE' }))
    await flush()
    assert.equal(f.instance.data.error.title, '当前路线已过期')
    assert.equal(f.instance.data.error.expiredRoute, true)
    assert.equal(f.instance.data.error.retryable, false)
    assert.equal(f.instance.data.detail, null)
    f.instance.onRetry()
    assert.equal(f.requests.length, 1)
    ...
    assert.equal(f.instance.data.error, null)
    assert.equal(f.requests[1].args.action, 'tripList')
    assert.equal(f.requests[1].args.type, 'all')
    assert.equal(f.requests[1].args.offset, 0)
    f.requests[1].resolve({ ok: true, items: [item('available', 'carpool')], hasMore: false, nextOffset: 1 })
    await flush()
    assert.equal(f.instance.data.items[0].id, 'available')
  }
  const wxml = fs.readFileSync(path.join(root, 'components/timeline-preview/timeline-preview.wxml'), 'utf8')
  assert.match(wxml, /<view wx:if="\{\{!error \|\| !error.expiredRoute\}\}" class="preview-header">/)
  const expiredState = wxml.match(/<view wx:if="\{\{error && error.expiredRoute\}\}"[\s\S]*?<\/view>/)[0]
  assert.match(expiredState, /当前路线已过期/)
  assert.match(expiredState, /bindtap="onFindAvailableTrips"[^>]*>查找当前可用拼车<\/button>/)
  assert.doesNotMatch(expiredState, /preview-intro|preview-back|detail-card|state-description/)
  assert.match(wxml, /<block wx:elif="\{\{mode === 'info'\}\}">/)
})

test('searching after a listed trip expires discards cached cards and the old type filter', async () => {
  const f = fixture({ kind: 'trip', type: 'request' })
  f.attach()
  f.requests[0].resolve({ ok: true, items: [item('ended', 'request')], hasMore: true, nextOffset: 10 })
  await flush()
  f.instance.onScroll({ detail: { scrollTop: 500 } })
  f.instance.onOpenDetail(open('request:ended'))
  f.requests[1].reject(Object.assign(new Error('not_found'), { code: 'UNAVAILABLE' }))
  await flush()
  f.instance.onFindAvailableTrips()
  assert.equal(f.instance.data.items.length, 0)
  assert.equal(f.instance.data.scrollTop, 0)
  assert.equal(f.instance.data.hasMore, false)
  assert.equal(f.instance.data.fromList, false)
  assert.equal(f.requests[2].args.type, 'all')
  assert.equal(f.requests[2].args.offset, 0)
})

test('network errors and unavailable trip lists never claim a shared route has expired', async () => {
  for (const code of ['LOAD_FAILED', 'TIMEOUT']) {
    const f = fixture({ kind: 'trip', type: 'carpool', id: 'active' })
    f.attach()
    f.requests[0].reject(Object.assign(new Error('private error'), { code }))
    await flush()
    assert.equal(f.instance.data.error.title, '暂时没有加载成功')
    assert.equal(f.instance.data.error.expiredRoute, undefined)
    assert.equal(f.instance.data.error.retryable, true)
    f.instance.onFindAvailableTrips()
    assert.equal(f.requests.length, 1)
    f.instance.onRetry()
    assert.equal(f.requests[1].args.action, 'tripDetail')
  }
  const list = fixture({ kind: 'trip' })
  list.attach()
  list.requests[0].reject(Object.assign(new Error('unavailable'), { code: 'UNAVAILABLE' }))
  await flush()
  assert.equal(list.instance.data.error.expiredRoute, undefined)
})
