...
const { createAnalyticsClient } = require('./analyticsClient')
...
function createAnalyticsSession(options = {}) {
  ...
  const client = options.client || createAnalyticsClient({ wx: wxApi, config, now, random, transport: options.transport })
  let sequence = 0
